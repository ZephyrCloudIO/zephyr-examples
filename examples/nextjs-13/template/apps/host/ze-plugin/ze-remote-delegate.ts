export const zeRemoteDelegate = new Promise(async (resolve, reject) => {
  const debug = Boolean(
    process.env.DEBUG &&
  ...
  if (!__resourceQuery) {
    reject("Running the delegate in a non-delegate-remote context. Missing '__resourceQuery' in the context.");
  }
...
export default zeRemoteDelegate;
