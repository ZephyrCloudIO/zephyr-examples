...
interface ZephyrWelcomeProps {
  ...
  bundlerLogo?: string;
  pills: string[];
  /** Optional content rendered between the hero and the "Get your own copy" block. */
  mid?: ReactNode;
}
...
export function ZephyrWelcome({ title, slug, bundlerName, frameworkLogo, bundlerLogo, pills, mid }: ZephyrWelcomeProps) {
  const ref = useRef<HTMLDivElement>(null);
  const [midSlot, setMidSlot] = useState<HTMLElement | null>(null);
  ...
  useEffect(() => {
    ...
    const logos = frameworkLogo
      + (bundlerLogo ? `<span class="sep">+</span>${bundlerLogo}` : '')
      + `<span class="sep">+</span><a href="https://zephyr-cloud.io" target="_blank">${z(56)}</a>`;

    const stackHtml = pills.map(p => `<span class="pill">${p}</span>`).join('');

    shadow.innerHTML = `<style>${CSS}</style>${HTML(title, slug, bundlerName, logos, stackHtml, cmd)}`;
    ...
    const slot = shadow.getElementById('zw-mid-slot');
    if (slot) setMidSlot(slot as HTMLElement);
  }, [title, slug, bundlerName, frameworkLogo, bundlerLogo, pills]);

  return (
    <>
      <div ref={ref} />
      {mid && midSlot ? createPortal(mid, midSlot) : null}
    </>
  );
}
...
<div id="zw-mid-slot" class="mid-slot"></div>
...
.mid-slot:not(:empty){margin-bottom:3rem}
