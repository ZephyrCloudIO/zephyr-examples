...
test.describe('Individual App Deployment Validation', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (deployedApps.length === 0) {
      throw new Error('No deployed applications found. Make sure build-packages has run successfully.');
    }
  });
  ...
  test('validate all deployed applications individually', async ({ page }) => {
    ...
    for (const app of deployedApps) {
      ...
      if (app.name.includes('turbo-')) {
        console.log(`  ⏭️  Skipping ${app.name} - turbo apps commented out until fixed`);
        results.push({ name: app.name, status: 'skipped', reason: 'turbo app' });
        continue;
      }
      ...
      try {
        ...
        if (validation.uniqueText.length > 0) {
          ...
          if (!foundUniqueText) {
            console.log(`    ⚠️  Expected text not found. Looking for: ${validation.uniqueText.join(', ')}`);
            console.log(`    📄 Page content snippet: "${pageText?.slice(0, 200)}..."`);
